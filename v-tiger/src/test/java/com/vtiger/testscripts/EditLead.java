...
public class EditLead extends BaseClass{
	...
	@Test
	public void editLead() throws EncryptedDocumentException, InvalidFormatException, IOException, InterruptedException
	{
	...
	Reporter.log("select the lead to be edited", true);
	l.getSelectedlead().click();
	
	Reporter.log("click on edit button", true);
	l.getEditBtn().click();
	l.getLastname().clear();
	
	Reporter.log("edit details", true);
	String detail = f.getExcelData("Lead", 2, 1);
	l.getLastname().sendKeys(detail);
	l.getSavebtn().click();
	
	String detail1 = l.getEditeddetail().getText();
	Assert.assertEquals(detail, detail1);
		}
}
